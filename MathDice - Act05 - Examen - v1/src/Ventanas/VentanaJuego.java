...
import javax.script.ScriptEngine;//libreria para la conversion cadena string->int
import javax.script.ScriptEngineManager;//libreria para la conversion cadena string->int
import javax.script.ScriptException;//libreria para la conversion cadena string->int
				...
				if(edit_sign==true && operaciones.getText().length()>2){
					for(int i=0;i<dado12.length;i++){
						if(dado12[i]==labelDado12.getIcon()){
					        ...
					        try {
					            sumatorio = (int) engine.eval(sumatorio_texto);
